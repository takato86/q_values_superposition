...
if __name__ == '__main__':
    ...
    if "Flexible" in args.env_id:
        ...
        env_to_wrap.env.set_init_states([(i, j) for i in range(1,6) for j in range(1,6)])
        # env_to_wrap.env.set_goal((3,6))
        env_to_wrap.env.set_goal((6,2))
        
        # 右上
        # initial_possible_next_goals = [62]
        # possible_next_goals = [62]
        # env_to_wrap.env.set_init_states([(i, j) for i in range(1,7) for j in range(7,12)])
        # env_to_wrap.env.set_goal((7, 9))

        # 左下
        # initial_possible_next_goals = [88]
        # possible_next_goals = [88]
        # env_to_wrap.env.set_init_states([(i, j) for i in range(7,11) for j in range(1,6)])
        # env_to_wrap.env.set_goal((10, 6))
        ...
        for episode in range(args.nepisodes):
            if episode == 1000:
                if len(possible_next_goals) > 0:
                    env.env.goal = possible_next_goals.pop()
                else:
                    env.env.goal = rng.choice(initial_possible_next_goals)
